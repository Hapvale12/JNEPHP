...
>
    <div class="flex items-center py-3 px-4 overflow-x-auto whitespace-nowrap max-w-7xl mx-auto">
        ...
        <!-- Inicio -->
        <a href="{{ url('/') }}" target="_blank" class="text-gray-600 hover:text-[#BE1717] transition-colors">
        ...
        @foreach ($segments as $index => $segment)
            ...
            @else
                <a href="{{ url($url) }}" target="_blank" class="text-gray-600 hover:underline transition-colors">
                    {{ $label }}
                </a>
            @endif
        @endforeach
